	...
	const debouncedFetchValues = createDebounce(100, () => {
		...
		getFolksonomyValues(fetch, requestedKey)
			...
			.catch((error) => {
				...
				// Error occurred in the latest request
				console.error('Failed to fetch possible values for key', requestedKey, error);
				possibleValues = []; // show empty state on error
			});
	});

	$effect(() => {
		// when newKey changes, clear stale values immediately and call the debounced fetch
		// Reference newKey to make it a reactive dependency
		void newKey;
		possibleValues = null; // clear suggestions immediately, don't wait for debounce
